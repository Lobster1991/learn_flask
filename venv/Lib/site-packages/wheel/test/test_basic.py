...
def teardown_module():
    ...
    base = pkg_resources.resource_filename('wheel.tests', '')
...
def build_wheel():
    """Build wheels from tests distributions."""
    for dist in test_distributions:
        pwd = os.path.abspath(os.curdir)
        distdir = pkg_resources.resource_filename('wheel.tests', dist)
        os.chdir(distdir)
        try:
...
def build_egg():
    """Build eggs from tests distributions."""
    for dist in test_distributions:
        pwd = os.path.abspath(os.curdir)
        distdir = pkg_resources.resource_filename('wheel.tests', dist)
        os.chdir(distdir)
...
def test_unpack():
    ...
    """
    for dist in test_distributions:
        distdir = pkg_resources.resource_filename('wheel.tests',
...
def test_no_scripts():
    ...
    dist = "complex-dist"
    basedir = pkg_resources.resource_filename('wheel.tests', dist)
...
def test_pydist():
    ...
    # XXX this tests may need manual cleanup of older wheels
    ...
    pymeta_schema = open_json(resource_filename('wheel.tests',
                                                'pydist-schema.json'))
    valid = 0
