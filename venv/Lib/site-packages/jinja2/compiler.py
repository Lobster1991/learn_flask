...
class DependencyFinderVisitor(NodeVisitor):
    """A visitor that collects filter and tests calls."""
...
class CodeGenerator(NodeVisitor):
    ...
    def visit_For(self, node, frame):
        ...
        # Create a fake parent loop if the else or tests section of a
        # loop is accessing the special loop variable and no parent loop
        # exists.
        if 'loop' not in aliases and 'loop' in find_undeclared(
           node.iter_child_nodes(only=('else_', 'tests')), ('loop',)):
        ...
        # if we have an extened loop and a node tests, we filter in the
        # "outer frame".
    ...
    def visit_Test(self, node, frame):
        ...
        if node.name not in self.environment.tests:
            self.fail('no tests named %r' % node.name, node.lineno)
        self.visit(node.node, frame)
        self.signature(node, frame)
        self.write(')')
