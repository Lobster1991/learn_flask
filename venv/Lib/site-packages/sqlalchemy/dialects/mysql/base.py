...
``REPEATABLE READ``, and ``SERIALIZABLE``::
    ...
    engine = create_engine(
                    "mysql://scott:tiger@localhost/tests",
...
``str`` type or ``bytes`` type, with data in the configured charset::
    ...
    e = create_engine("mysql+mysqldb://scott:tiger@localhost/tests?charset=utf8&use_unicode=0")
...
``nullable=False``.   The following example illustrates::
    ...
    e = create_engine("mysql://scott:tiger@localhost/tests", echo=True)
    m.create_all(e)
...
class MySQLDDLCompiler(compiler.DDLCompiler):
    ...
    def post_create_table(self, table):
        ...
        for opt in topological.sort([
            ...
            ('PARTITION_BY', 'PARTITIONS'),  # only for tests consistency
        ], opts):
            arg = opts[opt]
