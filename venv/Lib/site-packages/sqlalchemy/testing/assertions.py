...
def expect_warnings(*messages, **kw):
    ...
    Note that the tests suite sets SAWarning warnings to raise exceptions.
...
def emits_warning_on(db, *messages):
    """Mark a tests as emitting a warning on a specific dialect.
...
def uses_deprecated(*messages):
    """Mark a tests as immune from fatal deprecation warnings.
...
def global_cleanup_assertions():
    """Check things that have to be finalized at the end of a tests suite.
...
def _assert_no_stray_pool_connections():
    ...
    if pool._refs:
        ...
        print("Encountered a stray connection in tests cleanup: %s"
              % str(pool._refs))
...
class AssertsExecutionResults(object):
    ...
    def assert_list(self, result, class_, list):
        self.assert_(len(result) == len(list),
                     "result list is not the same size as tests list, " +
    ...
    def assert_unordered_result(self, result, cls, *expected):
        ...
        numbers of rows that the tests suite manipulates.
        """
